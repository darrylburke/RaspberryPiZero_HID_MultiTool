...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	then
		...
		for ((  i=0; i<${#info}; i++  )); do
			...
			if [ "$kbcode" != "" ]
			then
				echo "$kbcode" | /root/gadget/hid/hid-gadget-test $kb > /dev/null
			fi
		done
	elif [ "$cmd" == "ENTER" ] 
	then
		last_cmd="enter"
		echo "$last_cmd" | /root/gadget/hid/hid-gadget-test $kb > /dev/null
	
	elif [ "$cmd" == "DELAY" ] 
	then
		last_cmd="UNS"
		((info = info*1000))
		/root/gadget/hid/usleep $info
	...
	then
		...
		then
			...
			for ((  i=0; i<$info; i++  )); do
				...
				/root/gadget/hid/usleep $defdelay
			done
		fi
